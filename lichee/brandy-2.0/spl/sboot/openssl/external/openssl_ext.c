...
static  int __sunxi_publickey_dipatch(sunxi_key_t *pkey, u8 *buf, u32 len)
{
	...
	tmp_buf += asn1.head_len;		//����sequnceͷ��
	ret = __asn1_probe_data_head(tmp_buf, &asn1);
...
int sunxi_certif_history_check(u8 *name, uint8_t *bin_hash)
{
	int i;
	int name_len = strlen((char *)name);
	/*just in case have a very long name*/
	if (name_len > 15)
		return 0;
	//_a/_b suffix required
	if (name_len < 2)
		return 0;
	// check if is a a/b name
	if ((strcmp((char *)(name + name_len - 2), "_a")) &&
	    strcmp((char *)(name + name_len - 2), "_b"))
		return 0;
	for (i = 0; i < certif_history.history_count; i++) {
		/*
		 * exact same as verified hash for bin,
		 * and becomes a piar of  _a _b bin
		 * it's find to skip this one since we have
		 * verify the previous one
		 */
		if (memcmp(certif_history.hash[i], bin_hash, 32))
			continue;

		//same name except suffix _a or _b
		if (memcmp(certif_history.name, name, name_len - 2))
			continue;
	...
	if (i < 16) {
		certif_history.history_count++;
		memcpy(certif_history.name[i], name, 16);
		memcpy(certif_history.hash[i], bin_hash, 32);
	}
	return 0;
}
...
int sunxi_certif_verify_itself(sunxi_certif_info_t *sunxi_certif, u8 *buf, u32 len)
{
	...
	//��ȡ��Ҫǩ������
	//����sha256ʱ�����뱣֤�ڴ���ʼλ��16�ֽڶ��룬�����ȡ��32�ֽڶ���
	p_sign_to_calc = malloc(SUNXI_X509_CERTIFF_MAX_LEN);		//֤���д�ǩ�����ݿ϶�������8k
	//��ȡ��ǩ������
	memset(p_sign_to_calc, 0, SUNXI_X509_CERTIFF_MAX_LEN);
	sign_src_len = __certif_probe_signdata(p_sign_to_calc, SUNXI_X509_CERTIFF_MAX_LEN, buf, len);
	if(sign_src_len <= 0)
	{
	...
	if ((sunxi_certif->extension.extension_num == 1) &&
	    (sunxi_certif->extension.value_len[0] == 32)) {
		if (sunxi_certif_history_check(
			    sunxi_certif->extension.name[0],
			    sunxi_certif->extension.value[0]) == 1) {
			//we already verify a exact same certif
			//this one should be fine too, skip signature verify
			goto signature_pass;
		}
	}
	//����֤����ǩ����rsa
