...
//	int otag;
	...
	switch(it->itype)
		...
		*in = p;
		...
		if (!*pval && !ASN1_item_ex_new(pval, it))//--YXY	����it����һ���µ�item
			{
...
/**************************************************************************************/
		...
		for (i = 0, tt = it->templates; i < it->tcount; i++, tt++)//x509_CINF_seq_tt
			...
			pseqval = asn1_get_field_ptr(pval, seqtt);//���x509_CINF_seq_tt�����͵�ƫ����
		//	printf("%s\n",seqtt->field_name);
	...
	/*xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx*/
			if (!len)
				break;
			q = p;//֤���ƫ�Ƶ�ַ###
			if (asn1_check_eoc(&p, len))//����Ƿ�00��ͷ
				{
				if (!seq_eoc)
					{
			...
			ret = asn1_template_ex_d2i(pseqval, &p, len,		//���￪ʼ����template������ת��
							seqtt, isopt, ctx);
			if (!ret)
				{
				//errtt = seqtt;
...
static int asn1_check_tlen(long *olen, int *otag, unsigned char *oclass,
	...
	if (ctx && ctx->valid)
		...
		if (ctx)
			...
			ctx->pclass = pclass;
			ctx->ptag = ptag;
			ctx->hdrlen = p - q;//�������˶��ٸ��ֽڼ�0x30,0x82,0x04,0x52,4��
			ctx->valid = 1;
