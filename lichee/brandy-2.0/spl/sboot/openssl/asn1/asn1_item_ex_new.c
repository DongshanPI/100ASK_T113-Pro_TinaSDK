...
static int asn1_item_ex_combine_new(ASN1_VALUE **pval, const ASN1_ITEM *it,
	...
	switch(it->itype)
		...
		if (!combine)
			{
			*pval = OPENSSL_malloc(it->size);//���սṹ��ĵ�ַ
			if (!*pval)
				goto memerr;
			memset(*pval, 0, it->size);
		...
		for (i = 0, tt = it->templates; i < it->tcount; tt++, i++)//x509_seq_tt[]
			{
				pseqval = asn1_get_field_ptr(pval, tt);//����"���սṹ��"�е�ƫ�Ƶ�ַ	
			
				if (!ASN1_template_new(pseqval, tt))	//������item,����x509_seq_tt[]�е�xx_it
					goto memerr;
...
int ASN1_template_new(ASN1_VALUE **pval, const ASN1_TEMPLATE *tt)
	...
	ret = asn1_item_ex_combine_new(pval, it, tt->flags & ASN1_TFLG_COMBINE);//�ж���item�ṹ�����ͣ�����
	done:
	return ret;
	}
