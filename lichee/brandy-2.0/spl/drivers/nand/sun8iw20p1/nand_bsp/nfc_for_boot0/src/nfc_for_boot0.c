...
const __s16 para1[6][4] = { { 0x00, 0x06, 0x0a, 0x06 },
			    ...
			    { 0x00, 0x00, -0x1a, -0x1e },
			    { 0x00, 0x00, -0x20, -0x25 } };
const __s16 param0x40[10] = { 0x0, 0x0,  0x0,  0x1,  0x2,
			      0x3, 0x04, 0x05, 0x06, 0x07 };
const __s16 param0x41[12] = { 0x0,  0x0,  0x0,  0x1,  0x2,  0x3,
			      0x04, 0x05, 0x06, 0x07, 0x08, 0x0C };
...
void _dma_config_start(__u8 rw, __u32 buff_addr, __u32 len)
{
	...
	if (buff_addr & 0x3) {
		PRINT("[!!!!]%s(): buff addr(0x%x) is not 32bit aligned, "
		      "and it will be clipped to 0x%x",
		      "_dma_config_start", buff_addr, (buff_addr & 0x3));
	}
	reg_val = NFC_READ_REG(NFC_REG_CTL);
	reg_val &= (~(0x1 << 15));
	reg_val |= 0x1U << 14;
...
s32 ndfc_set_user_data_len(u32 udata_cnt)
{
	...
	u8 udata_len_mode[32]  = {
		0,
	}; /*user data lenth for every ecc block,udata_len is 4 bytes aligned*/
	u32 cfg       = 0;
	...
	for (i = 0; i < MAX_ECC_BLK_CNT; i++) {
		...
		cfg |= udata_len_mode[i]
		       << ((i & 0x07)
			   << 2); /*4 bit indecate one ecc block mode. a 32 register include 8 ecc block mode.*/
		if (((i + 1) & 0x07) == 0) {
...
__s32 _read_in_page_mode(NFC_CMD_LIST *rcmd, void *mainbuf, void *sparebuf,
			 __u8 read_mode)
{
	__s32 ret, ret1;
	__s32 i;
	...
	NFC_WRITE_REG(NFC_REG_CTL,
		      (NFC_READ_REG(NFC_REG_CTL)) | NFC_RAM_METHOD);

	/*set pagesize to 1K*/
	if (read_mode == 1) {
		page_size_temp = (NFC_READ_REG(NFC_REG_CTL) & 0xf00) >> 8;
		NFC_WRITE_REG(NFC_REG_CTL,
			      (NFC_READ_REG(NFC_REG_CTL)) | (0x3 << 8));
	...
	cfg |= (NFC_SEND_ADR | NFC_DATA_TRANS | NFC_SEND_CMD1 | NFC_SEND_CMD2 |
		NFC_WAIT_FLAG | NFC_DATA_SWAP_METHOD);
	...
	if (read_mode == 1) {
		...
		NFC_WRITE_REG(NFC_REG_ECC_CTL, ((NFC_READ_REG(NFC_REG_ECC_CTL) &
						 (~NFC_ECC_MODE)) |
						(0x0A << 8)));
	}
#ifdef FPGA_PLATFORM
	...
	if (read_mode == 1) {
		...
		NFC_WRITE_REG(NFC_REG_ECC_CTL,
			      (NFC_READ_REG(NFC_REG_ECC_CTL) &
			       (~NFC_ECC_MODE)) |
				      ecc_mode_temp);
		/*set pagesize to original value*/
		NFC_WRITE_REG(NFC_REG_CTL,
			      ((NFC_READ_REG(NFC_REG_CTL)) & (~NFC_PAGE_SIZE)) |
				      (page_size_temp << 8));
	}
...
********************************************************************************/
__s32 NFC_Read(NFC_CMD_LIST *rcmd, void *mainbuf, void *sparebuf,
	       __u8 read_mode, __u8 page_mode)
{
	__s32 ret;
...
__s32 NFC_ChangMode(NFC_INIT_INFO *nand_info)
{
	...
	if (nand_info->pagesize == 2) /*  1K  */
		...
		cfg |= (0x3 << 8);
	else if ((nand_info->pagesize > 16) &&
		 (nand_info->pagesize < 32)) /*  12K  */
		cfg |= (0x4 << 8);
...
/*
	 * There is a timing issue on V7 FPGA_PLATFORM.
	 * We need set serial_access_mode 0(normal mode) on V7 FPGA_PLATFORM.
	 */
...
__s32 _vender_set_param(__u8 *para, __u8 *addr, __u32 count)
{
	...
	for (i = 0; i < count; i++) {
		if ((read_retry_mode >= 0x0) && (read_retry_mode < 0x10)) {
			...
			/*set NFC_REG_CMD*/
			cfg = cmd_w;
			cfg |= (NFC_SEND_ADR | NFC_DATA_TRANS | NFC_ACCESS_DIR |
				NFC_SEND_CMD1);
			NFC_WRITE_REG(NFC_REG_CMD, cfg);
...
__s32 _vender_get_param_otp_hynix(__u8 *para, __u8 *addr, __u32 count)
{
	...
	cfg = (NFC_SEND_CMD1 | NFC_DATA_TRANS | NFC_ACCESS_DIR | NFC_SEND_ADR |
	       0x36);
	_wait_cmdfifo_free();
	NFC_WRITE_REG(NFC_REG_CMD, cfg);
	_wait_cmd_finish();
	...
	_wait_cmd_finish();
	/*get param data*/
	if ((NFC_READ_RAM_B(NFC_RAM0_BASE) != 0x08) ||
	    ((NFC_READ_RAM_B(NFC_RAM0_BASE + 1) != 0x08))) {
		ret = -1;
	}
	...
	for (j = 0; j < 8; j++) {
		...
		for (i = 0; i < 64; i++) {
			...
			param_reverse[i] = NFC_READ_RAM_B(NFC_RAM0_BASE +
							  128 * j + 64 + i);
...
__s32 _major_check_byte(__u8 *out, __u32 mode, __u32 level, __u8 *in,
			__u8 *in_inverse, __u32 len)
{
	__u32 bit, byte;
...
__s32 _get_read_retry_cfg(__u8 *rr_cnt, __u8 *rr_reg_cnt, __u8 *rr_tab,
			  __u8 *otp)
{
	...
	for (nbyte = 0; nbyte < rr_tab_size; nbyte++) {
		for (nset = 0; nset < 8; nset++) {
			...
			buf_inv[nset] = otp[16 + nset * rr_tab_size * 2 +
					    rr_tab_size + nbyte];
		}
...
__s32 _read_otp_info_hynix(__u32 chip, __u8 *otp_chip)
{
	...
	/*for (ndie = 0; ndie < 1; ndie++) */{
		...
		if ((ndie == 1) || (ndie == 2) || (ndie == 3)) {
			...
			cfg = 0;
			_set_addr(&abuf[0], 3);
			cfg = (NFC_SEND_CMD1 | NFC_SEND_ADR | (0x2 << 16) |
			       0x78);
			_wait_cmdfifo_free();
			NFC_WRITE_REG(NFC_REG_CMD, cfg);
			_wait_cmd_finish();
...

		{
			...
			_set_addr(&abuf[0], 1);
			cfg = (NFC_SEND_CMD1 | NFC_DATA_TRANS | NFC_ACCESS_DIR |
			       NFC_SEND_ADR | 0x36);
			_wait_cmdfifo_free();
			NFC_WRITE_REG(NFC_REG_CMD, cfg);
			_wait_cmd_finish();
...
__s32 _get_rr_value_otp_hynix(__u32 nchip)
{
	...
	ret = _get_read_retry_cfg(&rr_cnt_hynix_16nm, &rr_reg_cnt_hynix_16nm,
				  &hynix16nm_read_retry_otp_value[nchip][0][0],
				  otp_info_hynix_16nm);
	if (ret < 0) {
...
__s32 NFC_ReadRetry(__u32 chip, __u32 retry_count, __u32 read_retry_type)
{
	...
	if (read_retry_mode < 0x10) {
		/*for hynix read retry mode*/
		if ((read_retry_mode == 0) || (read_retry_mode == 1)) {
			if (retry_count == 0)
				ret = _vender_set_param(
					&read_retry_default_val[chip][0],
					&read_retry_reg_adr[0],
					read_retry_reg_num);
			else {
				for (i = 0; i < read_retry_reg_num; i++) {
					temp_val =
						(read_retry_default_val[chip][i] +
						 read_retry_val[retry_count - 1]
							       [i]);
					if (temp_val > 255)
						temp_val = 0xff;
					else if (temp_val < 0)
			...
			for (i = 0; i < read_retry_reg_num; i++)
				param[i] =
					hynix_read_retry_otp_value[chip]
								  [retry_count]
								  [i];
			...
			for (i		 = 0; i < read_retry_reg_num; i++)
				param[i] = hynix16nm_read_retry_otp_value
					[chip][retry_count][i];
		   ...
		   (read_retry_mode < 0x50)) {
		/*for micron readretry mode*/
		for (i		 = 0; i < read_retry_reg_num; i++)
			param[i] = (__u8)read_retry_val[retry_count - 1][i];
		ret = _vender_set_param(&param[0], &read_retry_reg_adr[0],
					read_retry_reg_num);
	} else {
...
__s32 NFC_ReadRetryInit(__u32 read_retry_type)
{
	...
	} else if (read_retry_mode == 0x41) {
		/*mode 0x41 L95B_128_256_512Gb_1Tb_2Tb_Async_Sync_NAND*/
		read_retry_reg_adr[0] = 0x89;
		for (i = 0; i < read_retry_cycle; i++) {
			for (j = 0; j < read_retry_reg_num; j++) {
...
__s32 NFC_GetDefaultParam(__u32 chip, __u8 *default_value,
			  __u32 read_retry_type)
{
	...
	if (read_retry_mode < 0x10) {
		/*hynix read retry mode*/
		if ((read_retry_mode == 0x0) || (read_retry_mode == 0x1)) {
			ret = _vender_get_param(
				&read_retry_default_val[chip][0],
				&read_retry_reg_adr[0], read_retry_reg_num);
			for (i = 0; i < read_retry_reg_num; i++) {
				default_value[i] =
					read_retry_default_val[chip][i];
			}
			...
			for (Count = 0; Count < 5; Count++) {
				PRINT("_vender_get_param_otp_hynix time %d!\n",
				      Count);
				ret = _vender_get_param_otp_hynix(
					&hynix_read_retry_otp_value[chip][0][0],
					&read_retry_reg_adr[0], 64);
				if (!ret)
					break;
			}
			if (ret)
			...
			for (i = 0; i < 8; i++) {
				for (j = 0; j < 4; j++) {
					default_value[4 * i + j] =
						hynix16nm_read_retry_otp_value
							[0][i][j];
				}
			}
		}
...
__s32 NFC_SetDefaultParam(__u32 chip, __u8 *default_value,
			  __u32 read_retry_type)
{
	__s32 ret;
	__u32 i;
		...
		for (i = 0; i < read_retry_reg_num; i++) {
			if ((read_retry_mode == 0x0) ||
				...
				default_value[i] =
				 ...
				 (read_retry_mode == 0x3))
				default_value[i] =
					hynix_read_retry_otp_value[chip][0][i];
			else if (read_retry_mode == 0x4)
				default_value[i] =
					hynix16nm_read_retry_otp_value[chip][0]
								      [i];
		}
		ret = _vender_set_param(default_value, &read_retry_reg_adr[0],
					read_retry_reg_num);
		return ret;
	} else if ((read_retry_mode >= 0x40) &&
		   (read_retry_mode < 0x50)) {
