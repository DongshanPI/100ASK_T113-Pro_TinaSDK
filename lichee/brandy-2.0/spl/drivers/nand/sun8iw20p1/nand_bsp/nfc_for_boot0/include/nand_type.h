...
struct __LogBlkType_t {
	...
	struct __SuperPhyBlkType_t
		PhyBlk; /*the super physic block number which the log block is mapping to*/
};
...
struct __BlkMapTblCache_t {
	...
	struct __SuperPhyBlkType_t *
		DataBlkTbl; /*the pointer to the data block table of the block mapping table*/
	struct __LogBlkType_t *
		LogBlkTbl; /*the pointer to the log block table of the block mapping table*/
	struct __SuperPhyBlkType_t *
		FreeBlkTbl; /*the pointer to the free block table of the block mapping table*/
...
struct __BlkMapTblCachePool_t {
	struct __BlkMapTblCache_t
		*ActBlkMapTbl; /*the pointer to the active block mapping table*/
	struct __BlkMapTblCache_t BlkMapTblCachePool
		[BLOCK_MAP_TBL_CACHE_CNT]; /*the pool of the block mapping table cache*/
	__u16 LogBlkAccessAge
		[MAX_LOG_BLK_CNT]; /*the time of accessing log block for the log block*/
...
struct __PageMapTblCache_t {
	...
	struct __PageMapTblItem_t
		*PageMapTbl; /*the pointer to the page mapping table*/
...
struct __PageMapTblCachePool_t {
	struct __PageMapTblCache_t *
		ActPageMapTbl; /*the poninter to the active page mapping table*/
	struct __PageMapTblCache_t PageMapTblCachePool
		[PAGE_MAP_TBL_CACHE_CNT]; /*the pool of the page mapping table cache*/
};
...
struct __LogicCtlPar_t {
	...
	struct __SuperPhyBlkType_t
		DataBlkNum; /*the number of the data block, which is accessed last time*/
	struct __SuperPhyBlkType_t
		LogBlkNum; /*the number of the log block, which is accessed last time*/
...
struct __NandPhyInfoPar_t {
	...
	__u32 DDRType;
	struct __OptionalPhyOpPar_t *
		OptionOp; /*the pointer point to the optional operation parameter*/
};

/*define the global paramter for nand flash driver to access all parameter*/
struct __NandDriverGlobal_t {
	struct __NandStorageInfo_t *
		NandStorageInfo; /*the pointer to the nand flash hardware information parameter*/
	struct __ZoneTblPstInfo_t *
		ZoneTblPstInfo; /*the pointer to the block mapping table information parameter*/
	struct __BlkMapTblCachePool_t *
		BlkMapTblCachePool; /*the pointer to the block mapping thable cache pool management parameter*/
	struct __PageMapTblCachePool_t *
		PageMapTblCachePool; /*the pointer to the page mapping table cache pool management parameter*/
	struct __LogicArchitecture_t *
		LogicalArchitecture; /*the pointer to the logical archtecture parameter*/
	struct __NandPageCachePool_t
		*PageCachePool; /*the pointer to the page cache pool parameter*/
};
...
#define DATA_PAGE_MARK                                                         \
	0x55 /*the physical page is used for storing the update data*/
#define TABLE_PAGE_MARK                                                        \
	0xaa /*the physical page is used for storing page mapping table*/

#define TABLE_BLK_MARK                                                         \
	0xaa /*the mark for the block mapping table block which is a special type block*/
#define BOOT_BLK_MARK                                                          \
	0xbb /*the mark for the boot block which is a special type block*/
...
#define ERR_ADDRBEYOND                                                         \
	20 /*the logical sectors need be accessed is beyond the logical disk*/
