...
void _InitNandPhyInfo(boot_nand_para_t *nand_info)
{
	...
	NandStorageInfo.OperationOpt =
		nand_info->OperationOpt &
		(NAND_LSB_PAGE_TYPE | NAND_RANDOM | NAND_READ_RETRY);
...
__s32 _CheckNandID(__u8 *pNandID)
{
	...
	while (1) {
		for (j = 0; j < 6; j++) {
			...
			if ((pNandID[j] != NandStorageInfo.NandChipId[j]) &&
			    (NandStorageInfo.NandChipId[j] != 0xff))
				break;
		}
...
__s32 BOOT_AnalyzeNandSystem(void)
{
	...
	if (SUPPORT_READ_RETRY &&
	    ((((READ_RETRY_TYPE >> 16) & 0xff) < 0x10) ||
	     ...
	     (((READ_RETRY_TYPE >> 16) & 0xff) ==
	      0x41))) {
		/*boot0 support hynix, micron readretry*/
...
__u32 Nand_Is_lsb_page(__u32 page)
{
	...
	if ((NAND_LSBPAGE_TYPE == 0x01)) {
		...
		if ((page == 0) || (page == 1))
			return 1;
		if ((page == PAGE_CNT_OF_PHY_BLK - 2) ||
		    (page == PAGE_CNT_OF_PHY_BLK - 1))
			return 0;
	...
	if (NAND_LSBPAGE_TYPE == 0x40) {
		/*micron 20nm 29f64g08cbaba*/
		if ((page == 0) || (page == 1))
			return 1;
		if ((page == PAGE_CNT_OF_PHY_BLK - 2) ||
		    (page == PAGE_CNT_OF_PHY_BLK - 1))
	...
	if (NAND_LSBPAGE_TYPE == 0x41) {
		/*micron 20nm 29f32g08cbada*/
		if ((page == 2) || (page == 3))
			return 1;
		if ((page == PAGE_CNT_OF_PHY_BLK - 2) ||
		    (page == PAGE_CNT_OF_PHY_BLK - 1))
	...
	if (NAND_LSBPAGE_TYPE == 0x43) { /* L04A */
		if ((page < 16) || ((page > 495) && (page < 512)))
			return 1;
		if (page % 2 == 0)
			return 1;
		if (page % 2 == 1)
	...
	if (NAND_LSBPAGE_TYPE == 0x44) { /* L06B */
		if (page < 64)
			return 1;
		if (page % 2 == 0)
			return 0;
...
__u32 NAND_GetLsbblksize(void)
{
	...
	if (NAND_Getlsbpage_type() == 0)
		return NandStorageInfo.PageCntPerPhyBlk *
		       NandStorageInfo.SectorCntPerPage * 512;
