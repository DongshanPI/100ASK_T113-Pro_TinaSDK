...
****************************************************************************/
void _add_cmd_list(NFC_CMD_LIST *cmd, __u32 value, __u32 addr_cycle, __u8 *addr,
		   __u8 data_fetch_flag, __u8 main_data_fetch, __u32 bytecnt,
		   __u8 wait_rb_flag)
{
	cmd->addr	    = addr;
...
*****************************************************************************/
void _cal_addr_in_chip(__u32 block, __u32 page, __u32 sector, __u8 *addr,
		       __u8 cycle)
{
	__u32 row;
	__u32 column;
...
__s32 _read_status(__u32 cmd_value, __u32 nBank)
{
	...
	addr_cycle = 0;
	_add_cmd_list(&cmd_list, cmd_value, addr_cycle, addr, 1, NFC_IGNORE, 1,
		      NFC_IGNORE);
...
__s32 _read_single_page(struct boot_physical_param *readop, __u8 read_mode)
{
	...
	if (Two_Row_Addr_Flag) {
		...
		_add_cmd_list(cmd_list, 0x00, 4, addr, NFC_NO_DATA_FETCH,
			      NFC_IGNORE, NFC_IGNORE, NFC_NO_WAIT_RB);
	} else {
		_cal_addr_in_chip(readop->block, readop->page, 0, addr, 5);
		_add_cmd_list(cmd_list, 0x00, 5, addr, NFC_NO_DATA_FETCH,
			      NFC_IGNORE, NFC_IGNORE, NFC_NO_WAIT_RB);
	}

	_add_cmd_list(cmd_list + 1, 0x05, NFC_IGNORE, NFC_IGNORE, NFC_IGNORE,
		      NFC_IGNORE, NFC_IGNORE, NFC_IGNORE);
	_add_cmd_list(cmd_list + 2, 0xe0, NFC_IGNORE, NFC_IGNORE, NFC_IGNORE,
		      NFC_IGNORE, NFC_IGNORE, NFC_IGNORE);
	_add_cmd_list(cmd_list + 3, 0x30, NFC_IGNORE, NFC_IGNORE, NFC_IGNORE,
		      NFC_IGNORE, NFC_IGNORE, NFC_IGNORE);
	list_len = 4;
	...
	if (SUPPORT_READ_RETRY && (read_mode == 0)) {
		if (((READ_RETRY_MODE >= 0x10) && (READ_RETRY_MODE < 0x30)) ||
		    ((READ_RETRY_MODE >= 0x40) &&
		     (READ_RETRY_MODE <
		      0x50))) /*toshiba or Samsung mode or micron*/
		...
		for (k = 0; k < READ_RETRY_CYCLE + 1; k++) {
			...
			NFC_RandomEnable();
			ret = NFC_Read(cmd_list, readop->mainbuf, sparebuf,
				       read_mode, NFC_PAGE_MODE);
			NFC_RandomDisable();
				...
				if ((READ_RETRY_MODE >= 0x10) &&
					...
					PHY_ResetChip(readop->chip);
					   ...
					   (READ_RETRY_MODE <
					    0x30)) {
					/*samsung mode*/
					NFC_SetDefaultParam(readop->chip,
							    default_value,
							    READ_RETRY_TYPE);
		...
		if (k > 0) {
			PHY_DBG("[Read_single_page] NFC_ReadRetry %d cycles, chip = %d, block = %d ",
				k, readop->chip, readop->block);
...
__s32 PHY_GetDefaultParam(__u32 bank)
{
	...
	if (!PageCachePool.PageCache0) {
		PageCachePool.PageCache0 =
			(__u8 *)MALLOC(SECTOR_CNT_OF_SUPER_PAGE * 512);
		if (!PageCachePool.PageCache0)
			return -1;
	}
	...
	if ((READ_RETRY_MODE == 2) || (READ_RETRY_MODE == 3)) {
		...
		for (i = uboot_start_block; i < uboot_next_block; i++) {
			...
			/*PHY_DBG("ret %d oob %x\n",ret,*((__u32 *)oob_buf));*/
			if ((ret >= 0) && (oob[0] == 0xff) &&
			    (oob[1] != 0xff) && (oob[2] != 0xff) &&
			    (oob[3] != 0xff)) {
				if (0 == check_nand_special_info(
						 (__u8 *)PHY_TMP_PAGE_CACHE +
						 nand_specialinfo_offset)) {
					otp_ok_flag = 1;
		...
		if (otp_ok_flag) {
			pdata = (__u8 *)(PHY_TMP_PAGE_CACHE +
					 nand_specialinfo_offset + 2);
			for (j			 = 0; j < 64; j++)
				default_value[j] = pdata[j];
			NFC_GetOTPValue(0, default_value, READ_RETRY_TYPE);
		} else {
			NFC_GetDefaultParam(chip, default_value,
					    READ_RETRY_TYPE);
			NFC_SetDefaultParam(chip, default_value,
					    READ_RETRY_TYPE);
		}
...
__s32 PHY_ChangeMode(__u8 serial_mode)
{
	...
	if (!PageCachePool.SpareCache) {
		PageCachePool.SpareCache =
			(__u8 *)MALLOC(SECTOR_CNT_OF_SUPER_PAGE * 4);
		if (!PageCachePool.SpareCache)
			return -1;
	}
	...
	if (!ndfc_dma_desc) {
		ndfc_dma_desc = (_ndfc_dma_desc_t *)MALLOC(
			sizeof(_ndfc_dma_desc_t) * 5);
		if (!ndfc_dma_desc)
			return -1;
	}
...
__s32 PHY_ResetChip(__u32 nChip)
{
	...
	_add_cmd_list(&cmd, 0xff, 0, NFC_IGNORE, NFC_NO_DATA_FETCH, NFC_IGNORE,
		      NFC_IGNORE, NFC_NO_WAIT_RB);
...
__s32 PHY_ReadNandId(__s32 nChip, void *pChipID)
{
	...
	_add_cmd_list(&cmd, 0x90, 1, &addr, NFC_DATA_FETCH, NFC_IGNORE, 6,
		      NFC_NO_WAIT_RB);
...
__s32 PHY_GetFeature(__s32 nChip, __u8 *addr, void *value)
{
	...
	NFC_SelectChip(nChip);
	_add_cmd_list(&cmd, 0xEE, 1, addr, NFC_DATA_FETCH, NFC_IGNORE, 4,
		      NFC_NO_WAIT_RB);
...
__s32 PHY_SetFeature(__s32 nChip, __u8 *addr, void *value)
{
	...
	NFC_SelectChip(nChip);
	_add_cmd_list(&cmd, 0xEF, 1, addr, NFC_DATA_FETCH, NFC_IGNORE, 4,
		      NFC_WAIT_RB);
...
__s32 Setup_Ddr_Nand_Force_To_Sdr_Para(boot_nand_para_t *nand_info)
{
	...
	flag = (nand_info->OperationOpt & (1 << 27)) ? 1 : 0;
	if (!flag) {
		return 0;
	}
