...
struct __ScanZoneInfo_t {
	...
	struct __SuperPhyBlkType_t
		ZoneTbl[BLOCK_CNT_OF_ZONE]; /*the zone table buffer*/
	struct __LogBlkType_t
		LogBlkTbl[MAX_LOG_BLK_CNT]; /*the log block mapping table buffer*/
};
...
struct __ScanDieInfo_t {
	...
	struct __ScanZoneInfo_t *
		ZoneInfo; /*the pointer to the zone table detail information of the whole die*/
};
...
#define DATA_TABLE_BITMAP                                                      \
	0xf /*the bitmap for check data block table of block mapping table*/
#define LOG_TABLE_BITMAP                                                       \
	0xf /*the bitmap for check log block table of block mapping table*/
#define DIRTY_FLAG_BITMAP                                                      \
	0x1 /*the bitmap for check dirty flag of block mapping table*/

/*define the offset of tables in one table group*/
#define DATA_TBL_OFFSET                                                        \
	0 /*page0 and page1 of a table group are used for data & free block table*/
