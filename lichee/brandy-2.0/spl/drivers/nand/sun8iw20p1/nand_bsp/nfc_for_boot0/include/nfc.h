...
/*#define NFC_REG_o_USER_DATA_LEN_X_BASE      0x0070*/
#define NFC_REG_USER_DATA_LEN(blk_num)                                         \
	__NFC_REG(NFC_REG_o_USER_DATA_LEN_X_BASE + 4 * blk_num)

#define NFC_REG_USER_DATA(blk_num)                                             \
	__NFC_REG(NFC_REG_o_USER_DATA_x_BASE + 4 * blk_num)
...
#define nfc_read_w(n) readl(n) /* word input */
#define nfc_write_w(n, c) writel(c, n) /* word output */

#define NFC_ERR_CNT_REG(blk_num)                                               \
	(NAND_IO_BASE + NFC_REG_o_ECC_CNT_x_BASE +                             \
	 (((blk_num)&0x1F) >> 2) * 0x4)
#define GET_ERR_CNT(blk_num)                                                   \
	((nfc_read_w(NFC_ERR_CNT_REG(blk_num)) >>                              \
	  ((((blk_num)&0x1F) % 4) * 8)) &                                      \
	 0xFF)

#define NFC_USER_DATA_REG(blk_num)                                             \
	(NFC_REG_o_USER_DATA_x_BASE + ((blk_num)&0x1F) * 0x4)
#define GET_USER_DATA(blk_num) (nfc_read_w(NFC_USER_DATA_REG(blk_num)))
#define SET_USER_DATA(blk_num, udata)                                          \
	(nfc_write_w(NFC_USER_DATA_REG(blk_num), udata))

/*define bit use in NFC_CTL*/
#define NFC_EN (1 << 0)
#define NFC_RESET (1 << 1)
#define NFC_BUS_WIDYH (1 << 2)
#define NFC_RB_SEL (0x3 << 3)
#define NFC_CE_CTL (1 << 6)
#define NFC_CE_CTL1 (1 << 7)
#define NFC_PAGE_SIZE (0xf << 8)
/*#define NFC_SAM					(1 << 12)*/
#define NFC_RAM_METHOD (1 << 14)
#define NFC_DMA_TYPE (1 << 15)
#define NFC_ALE_POL (1 << 16)
#define NFC_CLE_POL (1 << 17)
#define NF_TYPE (0x03 << 18)

#define NFC_DDR_REN (0x01 << 20)

#define NFC_CE_SEL (0xf << 24)
#define NFC_DDR_TYPE (1 << 28)
...
#define NFC_ADR_NUM_IN_PAGE_CMD (3 << 8)
...
#define NFC_SEND_RANDOM_CMD2_CTL (1 << 27)
...
#define NFC_RANDOM_CMD2 (0xff << 24)
...
#define NFC_RANDOM_EN (1 << 5)
#define NFC_RANDOM_DIRECTION (1 << 6)
#define NDFC_RANDOM_SIZE (1 << 7)
...
__s32 NFC_GetDefaultParam(__u32 chip, __u8 *defautl_value,
			  __u32 read_retry_type);
void NFC_GetOTPValue(__u32 chip, __u8 *otp_value, __u32 read_retry_type);
__s32 NFC_SetDefaultParam(__u32 chip, __u8 *defautl_value,
			  __u32 read_retry_type);
...
__s32 NFC_Read(NFC_CMD_LIST *rcmd, void *mainbuf, void *sparebuf,
	       __u8 dma_wait_mode, __u8 page_mode);
__s32 NFC_Read_1K(NFC_CMD_LIST *rcmd, void *mainbuf, void *sparebuf,
		  __u8 dma_wait_mode, __u8 page_mode);
__s32 NFC_Read_Seq(NFC_CMD_LIST *rcmd, void *mainbuf, void *sparebuf,
		   __u8 dma_wait_mode, __u8 page_mode);
__s32 NFC_Read_Spare(NFC_CMD_LIST *rcmd, void *mainbuf, void *sparebuf,
		     __u8 dma_wait_mode, __u8 page_mode);
__s32 NFC_Write(NFC_CMD_LIST *wcmd, void *mainbuf, void *sparebuf,
		__u8 dma_wait_mode, __u8 rb_wait_mode, __u8 page_mode);
__s32 NFC_Write_Seq(NFC_CMD_LIST *wcmd, void *mainbuf, void *sparebuf,
		    __u8 dma_wait_mode, __u8 rb_wait_mode, __u8 page_mode);
...
#define NFC_READ_REG(reg) readl(reg)
#define NFC_WRITE_REG(reg, data) writel(data, reg) /*(reg) = (data)*/
...
#define NFC_WRITE_RAM_B(ram, data)                                             \
	(*((volatile __u8 *)(NAND_IO_BASE + ram)) = (data))
#define NFC_READ_RAM_HW(ram) (*((volatile __u16 *)(NAND_IO_BASE + ram)))
#define NFC_WRITE_RAM_HW(ram, data)                                            \
	(*((volatile __u16 *)(NAND_IO_BASE + ram)) = (data))
#define NFC_READ_RAM_W(ram) (*((volatile __u32 *)(NAND_IO_BASE + ram)))
#define NFC_WRITE_RAM_W(ram, data)                                             \
	(*((volatile __u32 *)(NAND_IO_BASE + ram)) = (data))
