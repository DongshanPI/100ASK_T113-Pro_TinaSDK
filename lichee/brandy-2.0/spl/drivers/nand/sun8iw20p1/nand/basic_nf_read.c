...
__u32 NF_BLOCK_SIZE;
__u32 NF_BLK_SZ_WIDTH;
__u32 NF_PAGE_SIZE;
__u32 NF_PG_SZ_WIDTH;
__u32 BOOT1_LAST_BLK_NUM;
__u32 page_with_bad_block;
__u32 BOOT1_START_BLK;
...
__s32 NF_read(__u32 sector_num, void *buffer, __u32 N)
{
	...
	para.chip = 0;
	start_sct =
		g_mod(sector_num, NF_BLOCK_SIZE >> NF_SCT_SZ_WIDTH, &blk_num);
	para.block = blk_num;
	...
	if (residue != 0) {
		...
		if (NFB_BASE_PhyRead(&para) == FAIL)
			return NF_ERROR;
		for (j      = 0,
		    p       = (__u32 *)((__u32)buffer + NF_PAGE_SIZE * page_nr),
		    word_nr = residue * NF_SECTOR_SIZE >> 2,
		    q       = (__u32 *)page_buf;
		     j < word_nr; j++)
			*p++ = *q++;
	}
...
__u32 load_uboot_in_one_block_judge(__u32 length)
{
	...
	if (((length <= NF_BLOCK_SIZE) && (lsb_page_type == 0)) ||
	    ((length <= lsb_blk_size) && (lsb_page_type != 0)))
		return 1;
	else
		return 0;
}
