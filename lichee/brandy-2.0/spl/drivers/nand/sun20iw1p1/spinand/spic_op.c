...
{
	unsigned int rval = spi_rl(SPI_TCR) & (~(1 << 13));
...
{
	unsigned int rval = spi_rl(SPI_TCR) & (~(1 << 11));
...
{
	unsigned int rval = spi_rl(SPI_TCR) & (~(1 << 15));
...
{
	unsigned int rval = spi_rl(SPI_SDC)&(~(0x3f << 0));
...
{
	unsigned int rval = spi_rl(SPI_SDC);
...
{
	unsigned int rval = spi_rl(SPI_GCR);
...
__s32 Spic_init(__u32 spi_no)
{
	...
	if (nand_info.sample_delay == 0xaaaaffff) {
	...
	spi_wl(rval | spi_rl(SPI_TCR), SPI_TCR);
