...
void set_mmc_para(int smc_no, void *addr, u32 uboot_base)
{
	struct spare_boot_head_t *uboot_buf =
		(struct spare_boot_head_t *)uboot_base;

	/*
	 *memcpy((void *)(uboot_buf->boot_data.sdcard_spare_data),
	 *       (addr + SDMMC_PRIV_INFO_ADDR_OFFSET),
	 *       sizeof(struct boot_sdmmc_private_info_t));
	 */

	if (((struct sunxi_sdmmc_parameter_region *)mmc_arg_addr)->header.magic !=
			SDMMC_PARAMETER_MAGIC) {
		printf("mmc not para\n");
		return;
	}
...
#ifndef SUNXI_MMCDBG
	printf("tunning data addr:0x%x\n",
		(uint)(uboot_buf->boot_data.sdcard_spare_data));
#else
	u32 *p = NULL;
	int i  = 0;
...
static int mmc_clk_io_onoff(int sdc_no, int onoff,
			    const normal_gpio_cfg *gpio_info, int offset)
{
	unsigned int rval;
	struct sunxi_mmc_host *mmchost = &mmc_host[sdc_no];
...
static int mmc_config_clock_modex(struct sunxi_mmc_host *mmchost, unsigned clk)
{
	...
	if (mode == SUNXI_MMC_TIMING_MODE_1) {
		...
		if (readl(SUNXI_MMC_1X_2X_MODE_CONTROL_REG) & (0x1 << 3)) {
			rval = readl(&mmchost->reg->ntsr);
			rval |= (1U << 31);
			writel(rval, &mmchost->reg->ntsr);
		} else {
...
void mmc_update_host_caps_r(int sdc_no)
{
	...
	if (priv_info->ext_para1 & EXT_PARA1_1V8_GPIO_BIAS)
		writel(readl(SUNXI_PIO_BASE + GPIO_POW_MODE_REG) | (1 << 2),
		       SUNXI_PIO_BASE + GPIO_POW_MODE_REG);
#else
		ret = readl(SUNXI_PIO_BASE + GPIO_POW_MODE_VAL_REG);
		if (ret & (1 << 2)) {
